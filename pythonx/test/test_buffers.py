...
class TestBufferLists(unittest.TestCase):
    ...
    def test_create_buffer_with_existing_name_returns_that_buffer(self):
        """
        Test we can't create two different buffers with the same name in
        buffer lists
        """
        test_buffer = self.buffer_list.create_buffer('test', [])

        buffers.vim.eval = Mock(side_effect=['1', '4', '5'])
        test_buffer_2 = self.buffer_list.create_buffer('test', [])

        self.assertIs(test_buffer, test_buffer_2)
